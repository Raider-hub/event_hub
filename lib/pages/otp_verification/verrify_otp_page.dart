...
import 'package:event_hub/pages/otp_verification/provider/verify_otp_notifier.dart';
...
class VerifyEmailPage extends HookConsumerWidget {
  const VerifyEmailPage(this.userEmail, {Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context, ref) {
    ...
    ref.listen<VerifyOtpState>(verifyOtpProvider, (previous, current) {
      if (current.status == VerifyOtpStatus.loading) {
        context.loaderOverlay.show();
      } else if (current.status == VerifyOtpStatus.success) {
        context.loaderOverlay.hide();
        context.pushRoute(const ChoiceChipRoute());
      } else if (current.status == VerifyOtpStatus.error) {
        context.loaderOverlay.hide();
